...
import { Edge, Node , Handle, Position } from "react-flow-renderer";
import { CloseIcon, DragHandleIcon } from "@chakra-ui/icons";
import { storeSortData } from "../redux/TransactionReducer";
...
const SortNode = (props: any) => {
  ...
  useEffect(() => {
    if (transactionData?.sort?.[props?.id]) {
      const { column, condition } = transactionData?.sort[props?.id];
      setFilterData({
        column,
        condition,
      });
  ...
  return (
    ...
    >
      ...
      <div className="p-[6px] flex flex-col">
        ...
        >
          {(isConnected || transactionData?.sort?.[props?.id]) &&
      ...
      {filterData.column && filterData.condition && (
        <Button
          ...
          onClick={(e) => {
            e.preventDefault();
            if (
              !transactionData?.sort?.[props?.id] ||
              transactionData?.sort?.[props?.id]?.condition !==
                filterData.condition ||
              transactionData?.sort?.[props?.id]?.column !== filterData.column
            ) {
              dispatch(storeSortData({ [props?.id]: filterData }));
              props?.handleClickRun?.(props?.id, "sort", filterData);
            }
          }}
        >
          Run
