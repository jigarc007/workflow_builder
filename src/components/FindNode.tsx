...
const FindNode = (props: any) => {
  ...
  useEffect(() => {
    if (transactionData?.find?.[props?.id]) {
      const { column, condition, value } = transactionData?.find[props?.id];
      setFilterData({
        column,
        value,
        condition,
  ...
  return (
    ...
    >
      ...
      <div className="p-[6px] flex flex-col">
        ...
        {(isConnected || transactionData?.find?.[props?.id]) &&
          props?.columns?.length > 0 && (
            <Input
              backgroundColor="#2C2C47"
              color="white"
      ...
      {filterData.column && filterData.condition && filterData.value && (
        <Button
          ...
          onClick={(e) => {
            e.preventDefault();
            if (
              !transactionData?.find?.[props?.id] ||
              transactionData?.find?.[props?.id]?.condition !==
                filterData.condition ||
              transactionData?.find?.[props?.id]?.column !== filterData.column ||
              filterData.value !== transactionData?.find?.[props?.id]?.value
            ) {
              dispatch(storeFilterData({ [props?.id]: filterData }));
              props?.handleClickRun?.(props?.id, "find", filterData);
            }
          }}
        >
          Run
