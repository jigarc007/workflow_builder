...
import { storeMapData } from "../redux/TransactionReducer";
...
const MapNode = (props :any) => {
  ...
  useEffect(()=>{
    if(transactionData?.map?.[props?.id]){
      const {column}=transactionData?.map[props?.id]
      setFilterData({  column })
    }
  ...
  useEffect(()=>{
    if(edgesData.length>0){
      const edgeIndex=edgesData?.findIndex((edge: Edge)=>edge?.source===props.id || edge?.target===props.id);
  ...
  return (
    ...
    >
      ...
      <div className="p-[6px] flex flex-col">
        <Text className="text-sm mb-1 text-gray-300">Extract particular Column</Text>
        ...
        >
          {(isConnected || transactionData?.map?.[props?.id])  && props?.columns?.length>0 && optionValues?.column?.map((value: any,index: number)=>(
    ...
    type="button"
        ...
        onClick={(e)=>{
          e.preventDefault();
          if(!transactionData?.map?.[props?.id] || transactionData?.map?.[props?.id]?.column !== filterData.column){
            dispatch(storeMapData({[props?.id]:filterData}))
            props?.handleClickRun?.(props?.id,'map', filterData)
          }
        }}
      >Run</Button>
      }
