...
import { storeReduceData } from "../redux/TransactionReducer";
...
const ReduceNode = (props :any) => {
  const { data: transactionData } = useSelector((state: RootState) => state.transaction);
  ...
  useEffect(()=>{
    if(transactionData?.reduce?.[props?.id]){
      const {column,condition}=transactionData?.reduce[props?.id]
      setFilterData({
        column,
        condition
      })
  ...
  const [optionValues, setOptionValues] = useState({
    ...
    condition: ['Sum','Average','Max','Min'],
  });
  useEffect(()=>{
    console.log("edges===========>",edgesData)
    if(edgesData.length>0){
      const edgeIndex=edgesData?.findIndex((edge: any)=>edge?.source===props.id || edge?.target===props.id);
  ...
  const handleCloseClick=()=>{
    const filterNodeData: any[]=nodeData?.filter((node: any)=> node?.id!==props?.id);
    const filterEdgeData: any[]=edgesData?.filter((edge: any)=> (edge?.source!==props?.id || edge?.target!==props?.id));
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center p-[6px] border-b-[1px] border-[#7371b5]">
        ...
        <CloseIcon onClick={handleCloseClick} color={"rgb(156, 168, 179)"} width={"6px"} height={"6px"} />
      </div>
      {/* Column Name Label */}
      <div className="p-[6px] flex flex-col">
        ...
        >
          {(isConnected || transactionData?.reduce?.[props?.id])  && props?.columns?.length>0 && optionValues?.column?.map((value: any,index: number)=>(
          <option style={{color:"white", backgroundColor:"#2C2C47"}} key={`${index}_${value?.field}`} value={value.field}>{value?.field}</option>
        ...
        >
          ...
          <option style={{color:"white", backgroundColor:"#2C2C47"}} key={`${index}_${value}`} value={value}>{value}</option>
    ...
    {filterData.column && filterData.condition && 
    <Button
    type="button"
        backgroundColor="#2C2C47"
        style={{
        ...
        onClick={(e)=>{
          e.preventDefault();
          if(!transactionData?.reduce?.[props?.id] || transactionData?.reduce?.[props?.id]?.condition!==filterData.condition || transactionData?.reduce?.[props?.id]?.column !== filterData.column ){
            dispatch(storeReduceData({[props?.id]:filterData}))
            props?.handleClickRun?.(props?.id,'reduce', filterData)
          }
        }}
      >Run</Button>
      }
      ...
      <Handle
        ...
        onConnect={(params)=>{ console.log('connect the filter node')}}
        style={{
